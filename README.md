...
.sidebar{width:220px;flex-shrink:0;background:var(--bg2);border-right:1px solid var(--border);padding:24px 0;display:flex;flex-direction:column;position:fixed;top:0;left:0;height:100vh;z-index:50}
...
<script>
// ══════════════════════════════════════════════
// STORAGE (multi-user: cuentas en localStorage)
// ══════════════════════════════════════════════
const STORE_KEY = 'agentplan_db';
let db = {};          // { username: { pass, nombre, perfil, tareas, apuntes } }
let currentUser = null;  // username string
let D = null;         // shortcut to db[currentUser]

function loadDB(){ const r=localStorage.getItem(STORE_KEY); if(r) db=JSON.parse(r); }
function saveDB(){ localStorage.setItem(STORE_KEY,JSON.stringify(db)); }
function saveD(){ saveDB(); }

loadDB();
// Check session
const sessionUser = sessionStorage.getItem('ap_user');
...
function switchTab(tab){
  ['login','registro'].forEach(t=>{
    ...
    document.querySelectorAll('.auth-tab').forEach((el,i)=>el.classList.toggle('active',(i===0&&tab==='login')||(i===1&&tab==='registro')));
  });
}
...
function doLogin(){
  ...
  sessionStorage.setItem('ap_user',u);
  showErr('l-error','');
...
function doRegistro(){
  ...
  db[u]={ pass:btoa(p), nombre, perfil:null, tareas:[], apuntes:[] };
  saveDB();
  currentUser=u; D=db[u];
  sessionStorage.setItem('ap_user',u);
  showErr('r-error','');
...
function diffDots(n){ let h='<div class="diff-dots">'; for(let i=1;i<=5;i++) h+=`<div class="diff-dot${i<=n?' lit':''}"></div>`; return h+'</div>'; }
...
/*
  Rules per type+difficulty:
  Examen:   diff1=4h, diff2=6h, diff3=10h, diff4=15h, diff5=22h
  Proyecto: diff1=3h, diff2=5h, diff3=8h, diff4=14h, diff5=20h (done in 1-2 bursts)
  Lectura:  based on pages (40pg/h easy, 25pg/h hard), capped by difficulty
  Trabajo:  diff1=2h, diff2=4h, diff3=7h, diff4=11h, diff5=16h
  Tarea:    diff1=0.5h, diff2=1h, diff3=2h, diff4=3h, diff5=5h
  Otro:     same as Examen
*/
const H_BASE = {
  Examen:   [4,6,10,15,22],
  Proyecto: [3,5,8,14,20],
  Trabajo:  [2,4,7,11,16],
  Tarea:    [0.5,1,2,3,5],
  Lectura:  [0,0,0,0,0],  // calculated separately
  Otro:     [4,6,10,15,22],
};
// Cadencia: how many study sessions & how spread (days apart)
// Returns {sesiones, cadencia_desc}
function getCadencia(tipo, diff, hTotal, daysLeft){
  if(tipo==='Tarea'){
    return {sesiones:1, desc:'Se puede hacer en una sola sesión'};
  }
  if(tipo==='Proyecto'){
    const s = diff<=2 ? 1 : diff<=3 ? 2 : 3;
    return {sesiones:s, desc:`Concentrado en ${s} sesión${s>1?'es':''} intensiva${s>1?'s':''}`};
  }
  if(tipo==='Lectura'){
    const n = diff<=2 ? Math.ceil(hTotal/1.5) : diff<=3 ? Math.ceil(hTotal/1.2) : Math.ceil(hTotal);
    return {sesiones:n, desc:`Distribuido en ~${n} noches de lectura`};
  }
  if(tipo==='Trabajo'){
    const s = diff<=2 ? 2 : diff<=3 ? 3 : 4;
    return {sesiones:s, desc:`Planificado en ${s} sesiones de redacción`};
  }
  // Examen / Otro: temas difíciles necesitan más días para fijar memoria
...
function setDiff(v){
  currentDiff=v;
  document.querySelectorAll('#t-stars .star').forEach(s=>s.classList.toggle('lit',parseInt(s.dataset.v)<=v));
  recalcEstimate();
}
setDiff(3);
...
function recalcEstimate(){
  ...
  // Show/hide paginas field
  document.getElementById('grp-paginas').style.display=(tipo==='Lectura'?'block':'none');
  if(!fecha){ document.getElementById('hrs-estimate').style.display='none'; return; }
  const daysLeft=daysUntil(fecha);
  let hTotal;
  if(tipo==='Lectura'){
    if(!paginas){ document.getElementById('hrs-estimate').style.display='none'; return; }
    const pgPerHr=[50,40,30,22,15][currentDiff-1];
  ...
  document.getElementById('hrs-estimate').style.display='block';
}
...
function renderDashboard(){
  ...
  if(!ses.length){ el.innerHTML='<div class="empty"><div>Sin sesiones hoy</div></div>'; return; }
...
function taskHTML(t,mini=false){
  ...
  return `<div class="task-item">
    ...
    <div class="task-info">
      <div class="task-name${t.done?' done':''}">${t.nombre}</div>
...
function guardarTarea(){
  ...
  if(!hTotal){
    if(tipo==='Lectura'&&paginas){ const pgPerHr=[50,40,30,22,15][currentDiff-1]; hTotal=Math.ceil(paginas/pgPerHr); }
  ...
  D.tareas.push({
    ...
    done:false, creada:today().toISOString().slice(0,10)
  });
  saveD();
...
function calcPlan(){
  const p=D.perfil;
  const hd=Array.isArray(p.horas_dia)?p.horas_dia:[4,4,4,4,4,0,0];
  ...
  pend.forEach(t=>{
    ...
    for(let i=0;i<14;i++){
      ...
      if((hd[dow]||0)>0&&d<entrega) days.push({key:d.toISOString().slice(0,10),hd:hd[dow]});
    }
    if(!days.length) return;
    // Smart scheduling: for Proyecto/Tarea, cluster sessions; for Lectura/Examen, space out
    const tipo=t.tipo;
    let {sesiones}=getCadencia(tipo,t.diff,hRest,days.length);
    sesiones=Math.min(sesiones,days.length);
    // Pick which days to use
    let chosenDays;
    if(tipo==='Proyecto'||tipo==='Tarea'){
      // cluster near deadline
    ...
    // Assign hours: harder topics get more hours in early sessions
    let restante=hRest;
    chosenDays.forEach(({key,hd:maxHd})=>{
      if(restante<=0) return;
      const h=Math.min(Math.ceil(restante/chosenDays.length*10)/10, restante, maxHd);
      if(h>0){ if(!agenda[key])agenda[key]=[]; agenda[key].push({nombre:t.nombre,materia:t.materia,horas:Math.round(h*10)/10}); restante-=h; }
    });
  });
  return agenda;
}
function renderPlan(){
  const plan=calcPlan();
  const hd=Array.isArray(D.perfil.horas_dia)?D.perfil.horas_dia:[4,4,4,4,4,0,0];
  let html='';
  for(let i=0;i<14;i++){
    const d=new Date(today()); d.setDate(d.getDate()+i);
    const dow=d.getDay()===0?6:d.getDay()-1;
    ...
    html+=`<div class="plan-day"><div class="plan-day-hdr"><span>${DIAS_FULL[dow]}</span><span class="plan-day-date">${d.toLocaleDateString('es-ES',{day:'numeric',month:'short'})}</span>${ses.length?`<span class="plan-day-hrs">${Math.round(totalH*10)/10}h</span>`:'<span class="plan-day-hrs" style="color:var(--text3)">libre</span>'}</div>${ses.map(s=>`<div class="plan-session"><div class="plan-dot"></div><span>${s.nombre}</span><span class="plan-mat">${s.materia}</span><span class="plan-hrs">${s.horas}h</span></div>`).join('')}${!ses.length?'<div class="plan-session" style="color:var(--text3)">Sin sesiones</div>':''}</div>`;
  }
...
function guardarApunte(){
  ...
  const ap={materia,titulo,fecha:today().toISOString().slice(0,10),tipo:apTipo,tareaId};
